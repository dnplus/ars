...
- Use the CLI flow: `npx ars prepare youtube <series>/<epId>` first, then wait for explicit human confirmation before any publish step.
- After confirmation, run `npx ars publish youtube <series>/<epId>` with any user-approved flags such as `--dry-run`, `--yes`, or `--privacy`.
...
- Report the result and next follow-up actions, such as social prepare/publish if requested.
