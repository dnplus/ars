...
4. **Card text ≠ narration.** For each step, compare visible `step.data` text fields (for example `content`, `cardTitle`, `title`, `points`, `cta`, `ctaButtons`, or `chart` labels) against the same step's narration. If the card text is more than ~50% verbatim narration, rewrite the card into key phrases / numbers / anchors. The card is the visual hook; the narration carries the story.
