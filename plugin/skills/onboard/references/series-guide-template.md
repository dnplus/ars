...
When writing `SERIES_GUIDE.md` during Phase 2 customize, use this structure. This file is not just branding. It is the series background brief that later planning, build, review fixes, and reflection work should inherit.
...
Newly onboarded series should consolidate everything into one `SERIES_GUIDE.md`. Build / apply-review still read `STYLING.md` (and `VOICE.md`, `persona.md`) when present, for backward compatibility.

## Optional sections (generated by the deep-dive)

Quick customize produces the basic sections below. The optional deep-dive appends the optional sections at the end of `SERIES_GUIDE.md`. Sections corresponding to skipped deep-dive questions are simply omitted — never write empty placeholder sections.
...
<!-- Optional sections below are generated by the voice/card deep-dive. Omit any section the user skipped. -->
