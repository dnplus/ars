...
- When the user explicitly says review is done (or there is no audio round needed, e.g. a shorts with pre-approved voice), run `npx ars review close <epId>`, tell them the next step is `/ars:prepare-youtube <epId>`, then call `TaskStop` to end the monitor.
